...
### 증상
- 가상계좌 결제 시에도 주문 상태가 바로 `COMPLETED`로 변경되는 문제 발생
- 가상계좌는 입금 대기 상태여야 하는데 즉시 완료 처리됨

### 문제 코드
```python
def _handle_payment_success(request, response: dict):
    """결제 성공 처리"""
    request.payment_status = PaymentStatus.PAYMENT_COMPLETED  # 무조건 COMPLETED로 설정
...
{
  "mId": "twecomm49m7",
  "status": "WAITING_FOR_DEPOSIT",  // 실제 상태는 입금 대기
  "method": "가상계좌",
  "virtualAccount": {
    "accountNumber": "X9018612288905",
...
### 문제점
1. 결제 상태를 확인하지 않고 무조건 완료 처리
2. 가상계좌 특성을 고려하지 않은 상태 관리
3. `approvedAt` 값이 없는 경우에 대한 처리 부재
...
def _handle_payment_success(request, response: dict):
    """결제 성공 처리"""
    if response.get('status') == 'WAITING_FOR_DEPOSIT':  # 가상계좌 입금 대기 상태 체크
        request.payment_status = PaymentStatus.PAYMENT_PENDING
        request.order_status = OrderStatus.ORDER_PENDING
    else:  # 일반 결제 완료
...
### 주요 변경사항
1. 결제 상태 분기 처리 추가
   - `WAITING_FOR_DEPOSIT` 상태 확인
   - 가상계좌: `PAYMENT_PENDING` & `ORDER_PENDING`
   - 일반결제: `PAYMENT_COMPLETED` & `ORDER_COMPLETED`

2. `approved_at` 처리 개선
   - None 값일 경우 현재 시간으로 설정
   - KST 시간대 적용
